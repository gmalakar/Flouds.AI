...
def export_and_optimize_onnx(
    ...
    def _verify_model(model_path: str):
        ...
        try:
            ...
            print("Warning: Skipping onnx.checker.check_model due to MemoryError (model is too large).")
    ...
    def _optimize_model(task: str, optimization_level: int):
        ...
        if optimize:
            ...
            if has_decoder:
                ...
                print(f"Decoder model optimized and saved as: {optimized_decoder_model.name}")
                del decoder_model
    ...
    if task == "seq2seq-lm":
        ...
        if use_t5_encoder:
            print(f"Exporting model {model_name} for task {task} (T5 encoder-only export)...")
