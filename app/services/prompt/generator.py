...
def run_seq2seq_generation(
    ...
    try:
        ...
        input_text = prepare_input_text(
            request, getattr(model_config, "prepend_text", None)
        )
        ...
        return SummaryResults(
            summary="", message=f"Error generating text: {str(e)}", success=False
        )
...
def _run_encoder(
    ...
    if "attention_mask" in inputs:
        try:
            mask = np.ascontiguousarray(np.asarray(inputs["attention_mask"])).astype(
                np.int64
            )
        except Exception:
    ...
    try:
        ...
        seq_len = (
            int(np.asarray(inputs["input_ids"]).shape[1])
        ...
        for sin in sess_inputs:
            ...
            if any(
                s in name.lower() for s in ["past_key_values", "past", ".key", ".value"]
            ):
                try:
                    shape = [
