...
class SentenceTransformer(BaseNLPService):
    ...
    _prepare_embedding_resources = staticmethod(
        lambda model: resource_manager.prepare_embedding_resources(
            model, SentenceTransformer
        )
    )
    _override_config_with_request = staticmethod(
        resource_manager.override_config_with_request
    )
        ...
        **kwargs: Any,
        ...
        try:
            model_config, tokenizer, session = (
                resource_manager.prepare_embedding_resources(model, SentenceTransformer)
            )
