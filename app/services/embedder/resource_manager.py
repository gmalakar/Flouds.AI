...
def prepare_embedding_resources(model: str, base_service_class: Any) -> Tuple[Any, Any, Any]:
    ...
    if not BaseNLPService._validate_model_availability(
        ...
        raise ModelNotFoundError(f"Model '{model}' not available or model files missing")
...
def load_tokenizer(model_to_use_path: str, model_config: Any, base_service_class: Any) -> Any:
    ...
    tokenizer = base_service_class._get_tokenizer_threadsafe(model_to_use_path, use_legacy)
    if not tokenizer:
        raise TokenizerError(f"Failed to load tokenizer: {model_to_use_path}")
    return tokenizer


def load_session(model_to_use_path: str, model_config: Any, base_service_class: Any) -> Any:
    ...
    model_path = base_service_class._get_embedding_model_path(model_to_use_path, model_config)
...
def get_model_filename(
    ...
    return BaseNLPService._get_model_filename(model_config, model_type, fallback_to_regular)
    ...
    for field in override_fields:
        if field in request_params:
            if request_params[field] is not None:
                ...
                logger.debug(f"Using request parameter for {field}: {request_params[field]}")
