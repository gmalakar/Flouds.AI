...
from app.services.embedder_service import SentenceTransformer, STOP_WORDS
...
@patch("app.services.embedder_service.SentenceTransformer._get_tokenizer", return_value=DummyTokenizer())
@patch("app.services.embedder_service.SentenceTransformer._get_encoder_session", return_value=DummySession())
def test_embed_text_success(mock_session, mock_tokenizer, mock_config, dummy_model_config):
    ...
    response = SentenceTransformer.embed_text(text, "dummy-model", projected_dimension=8)
    ...
    for chunk in response.results:
        ...
        assert all(isinstance(x, (float, np.floating, np.float32, np.float64)) for x in chunk.vector)
...
def test_small_text_embedding_returns_flat_list(dummy_model_config):
    embedding = SentenceTransformer._small_text_embedding(
        "Hello world", dummy_model_config, DummyTokenizer(), DummySession(), projected_dimension=8
    )
    assert isinstance(embedding, list)
    assert len(embedding) == 8
    assert all(isinstance(x, (float, np.floating, np.float32, np.float64)) for x in embedding)
...
def test_embed_text_handles_exception(monkeypatch, dummy_model_config):
    ...
    monkeypatch.setattr(SentenceTransformer, "_get_model_config", lambda *a, **k: dummy_model_config)
    monkeypatch.setattr(SentenceTransformer, "_get_tokenizer", lambda *a, **k: DummyTokenizer())
    monkeypatch.setattr(SentenceTransformer, "_get_encoder_session", lambda *a, **k: DummySession())
    monkeypatch.setattr(SentenceTransformer, "_split_text_into_chunks", raise_exc)
    response = SentenceTransformer.embed_text("fail test", "dummy-model", projected_dimension=8)
